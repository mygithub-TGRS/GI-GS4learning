...
// Forward: 17-element tuple (original 15 + sampleBuffer + num_buckets)
std::tuple<int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
...
RasterizeGaussiansCUDA(
	...
	torch::Tensor binningBuffer = torch::empty({0}, options.device(device));
	torch::Tensor imgBuffer = torch::empty({0}, options.device(device));
	torch::Tensor sampleBuffer = torch::empty({0}, options.device(device));  // NEW
	std::function<char*(size_t)> geomFunc = resizeFunctional(geomBuffer);
	std::function<char*(size_t)> binningFunc = resizeFunctional(binningBuffer);
	std::function<char*(size_t)> imgFunc = resizeFunctional(imgBuffer);
	std::function<char*(size_t)> sampleFunc = resizeFunctional(sampleBuffer);  // NEW

	int rendered = 0;
	int num_buckets = 0;  // NEW: set by forward
	if(P != 0) {
		int M = 0;
		if(sh.size(0) != 0) {
			M = sh.size(1);
	...
	return std::make_tuple(
		...
		binningBuffer,
		imgBuffer,
		sampleBuffer,   // NEW: sample state buffer (byte tensor)
		num_buckets,    // NEW: total bucket count (plain int)
		out_opacity,
		out_depth,
		out_normal,
		out_normal_view,
...
RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& sampleBuffer,   // NEW
	const int num_buckets,               // NEW
	const int R,
	const bool debug
) {
	const int P = means3D.size(0);
	...
	if(P != 0) {
		CudaRasterizer::Rasterizer::backward(P, degree, M, R,
			...
			reinterpret_cast<char*>(sampleBuffer.contiguous().data_ptr()),  // NEW
