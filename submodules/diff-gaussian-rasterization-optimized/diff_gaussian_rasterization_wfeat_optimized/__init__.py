...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        args = (
            ...
            metallic,
            feature,
            feature.shape[1] if feature.numel() > 0 else 0,
            scales,
            rotations,
            cov3Ds_precomp,
            sh,
        ...
        if raster_settings.debug:
            ...
            try:
                (
                    ...
                    binningBuffer,
                    imgBuffer,
                    sampleBuffer,    # NEW: sample state buffer
                    num_buckets,     # NEW: total bucket count (int)
                    opacity_map,
                    depth,
                    out_normal,
                    out_normal_view,
        ...
        ctx.num_buckets = num_buckets   # NEW: store bucket count for backward
        ctx.save_for_backward(
            ...
            binningBuffer,
            imgBuffer,
            sampleBuffer,    # NEW: save sample state for backward
        )
        return (
            color,
            radii,
    ...
    def backward(
        ...
        num_buckets = ctx.num_buckets   # NEW
...
class _SSR(torch.autograd.Function):
    ...
    def backward(
        ...
        None, None, None, torch.Tensor, torch.Tensor, torch.Tensor, None,
    ]:
        (
            normal,
            pos,
