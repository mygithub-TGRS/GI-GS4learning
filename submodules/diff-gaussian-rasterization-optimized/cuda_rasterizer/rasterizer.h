...
namespace CudaRasterizer
{
	...
	public:
		...
		// Forward pass. Returns num_rendered and fills sampleBuffer (for backward).
		// num_buckets is set by this function and must be passed to backward.
		static int forward(
			std::function<char* (size_t)> geometryBuffer,
			std::function<char* (size_t)> binningBuffer,
			std::function<char* (size_t)> imageBuffer,
			std::function<char* (size_t)> sampleBuffer,   // NEW: sample state buffer
			int& num_buckets,                              // NEW: total bucket count (out)
			const int P, int D, int M,
			const float* background,
			const int width, int height,
			const float* means3D,
		...
		// Backward pass. Receives sampleBuffer from forward and num_buckets.
		static void backward(
			const int P, int D, int M, int R,
			const int num_buckets,                         // NEW: total bucket count
			const float* background,
			const int width, int height,
			const float* means3D,
			const float* shs,
			...
			char* binning_buffer,
			char* img_buffer,
			char* sample_buffer,                           // NEW: sample state buffer
